...
const SmartPaymentsPage = () => {
  ...
  // Invoice draft coming from Invoices page
  ...
  // Pick up invoice payload from Invoices page
  useEffect(() => {
    const payload = location.state?.invoicePayload;
    if (payload) {
      setInvoiceDraft(payload);
      setSelectedClient(String(payload.clientId));
      setAppliedFilters((prev) => ({ ...prev, clientId: String(payload.clientId) }));
      // Remove state from history to avoid reusing on navigation
  ...
  // If an invoice draft arrives, auto-load payments for that client
  useEffect(() => {
    if (invoiceDraft?.clientId) {
      loadPayments({ clientId: invoiceDraft.clientId });
      loadClientInvoices(invoiceDraft.clientId);
    }
  }, [invoiceDraft]);
  ...
  const loadClientInvoices = async (clientId) => {
    ...
    try {
      const response = await getInvoicesForClient(clientId);
      if (response.success) {
  ...
  const handleApplyInvoiceDraft = async () => {
    if (!invoiceDraft) return;
    setApplyingInvoice(true);
    try {
      // First, notify backend that invoice is received (audit/logging)
      await applyInvoiceDraft({ ...invoiceDraft, invoiceStatus: "CREATED", sourceModule: "invoicesPage" });

      // Persist each invoice as a payment record
      const results = [];
      ...
      showNotification(`Applied ${results.length} invoice(s) to Smart Payments`, "success");
      setInvoiceDraft(null);
      loadPayments({ clientId: selectedClient || invoiceDraft.clientId });
      loadClientInvoices(selectedClient || invoiceDraft.clientId);
    } catch (error) {
      console.error("Error applying invoice draft:", error);
      showNotification(error.response?.data?.error || "Failed to apply invoice", "error");
    } finally {
        ...
        >
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2, gap: 2, flexWrap: "wrap" }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {invoiceDraft.clientName || getClientName(invoiceDraft.clientId)} • {invoiceDraft.fromDate} → {invoiceDraft.toDate}
              </Typography>
            </Box>
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button
                variant="contained"
                color="success"
                onClick={handleApplyInvoiceDraft}
                disabled={applyingInvoice}
                sx={{ textTransform: "none", fontWeight: 600 }}
              >
                {applyingInvoice ? <CircularProgress size={20} color="inherit" /> : "Apply Invoice"}
              </Button>
              <Button
                variant="outlined"
                color="inherit"
          ...
          <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap", mb: 2 }}>
            <Chip label={`Invoices: ${invoiceDraft.invoices?.length || 0}`} color="primary" />
            <Chip label={`Total: ${formatCurrency(invoiceDraft.totalAmount || 0)}`} color="secondary" />
          </Box>
              ...
              <TableBody>
                {(invoiceDraft.invoices || []).map((inv) => (
                  <TableRow key={inv.invoice_id || inv.id}>
                    ...
                    <TableCell>{formatCurrency(inv.total_amount || 0)}</TableCell>
      ...
      {selectedClient && (
        <Paper sx={{ p: 2, mb: 3, borderRadius: 2, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Typography variant="h6" sx={{ fontWeight: 600 }}>Invoices for Client</Typography>
            <Chip label={`Count: ${clientInvoices.length}`} />
          </Box>
          <TableContainer>
            <Table size="small">
              <TableHead>
              ...
              <TableBody>
                {invoicesLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} align="center">No invoices found for client</TableCell>
                  </TableRow>
                ) : (
                  clientInvoices.map((inv) => (
                    <TableRow key={inv.id}>
                      <TableCell>{getClientName(selectedClient)}</TableCell>
